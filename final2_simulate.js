...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const queryPath = `${path}?keywords=${encodeURIComponent(keyword)}&device=${device||'pc'}&target=QUERY`;
    const data = await new Promise((resolve, reject) => {
      ...
      doReq({
        hostname: 'api.naver.com', port: 443, path: queryPath, method: 'GET',
        headers: {
    ...
    if (budget && Array.isArray(result)) {
      const b = parseInt(budget);
      result.forEach(item => {
        const cpc = item.avgMonthlyPcClkCost || 0;
        item.estimatedClicks = cpc > 0 ? Math.floor(b / cpc) : 0;
      });
    }
